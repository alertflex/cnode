...
@Stateless
@Path("playbook")
public class PlaybookREST {

    private static final Logger logger = LoggerFactory.getLogger(PlaybookREST.class);
    ...
    public Response push(@PathParam("id") String id, @Context SecurityContext sc) {
        ...
        if (user != null && id != null && !id.isEmpty()) {
            ...
            Playbook p = playbookFacade.findPlaybookByName(user.getRefId(), id);
            
            String reportUuid = UUID.randomUUID().toString();

            if (p != null) {

                sendPlaybookToMQ(p, reportUuid);
    ...
    public void sendPlaybookToMQ(Playbook p, String r) {
        ...
        try {
            ...
            message.setStringProperty("report_uuid", r);
