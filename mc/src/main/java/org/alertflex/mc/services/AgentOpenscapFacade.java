...
public class AgentOpenscapFacade extends AbstractFacade<AgentOpenscap> {
    ...
    public List<AgentOpenscap> findOpenscap(String ref, String node, String agent) {
        
        List<AgentOpenscap> l = null;
        
        try {
            em.flush();
            
            Query listQry = em.createQuery(
                "SELECT a FROM AgentOpenscap a WHERE a.refId = :ref AND a.nodeId = :node AND a.agent = :agent").setParameter("ref", ref).setParameter("node", node).setParameter("agent", agent);
        
            // Enable forced database query
            listQry.setHint("javax.persistence.cache.retrieveMode", CacheRetrieveMode.BYPASS);
            l =  (List<AgentOpenscap>) listQry.getResultList();
